...
First activate your conda environment using the terminal command `conda activate milkra` (or on more recent versions `activate milkra`). After this, navigate to the directory of the script, `cd path/to/MilkRA/preprocessing` (note this may require going back up the filepath; this can be performed using `cd ..`). To run the script, use the terminal command `python main.py`. There are several command-line arguments available:
